...
def get_thong_tin_don_hang_tndsoto(id):
    donhang = DonDatHang.objects.get(id=id)
    otobaohiem = NguoiDuocBaoHiem.objects.get(ma_don_hang_bihama=donhang.ma_don_hang_bihama)
    ...
    data_dict["bien_so_xe"] = otobaohiem.bien_so_xe
    data_dict["loai_nguoi_mua"] = otobaohiem.loai_nguoi_mua
    data_dict["muc_dich_su_dung"] = otobaohiem.muc_dich_su_dung
    data_dict["trong_tai"] = otobaohiem.trong_tai
    data_dict["so_tien_bao_hiem_tnds_tu_nguyen"] = otobaohiem.so_tien_bao_hiem_tnds_tu_nguyen
    data_dict["images_dangky_mattruoc"] = otobaohiem.images_dangky_mattruoc
    data_dict["images_dangky_matsau"] = otobaohiem.images_dangky_matsau
    if otobaohiem.ngay_hieu_luc is not None:
        data_dict["ngay_hieu_luc"] = otobaohiem.ngay_hieu_luc.strftime('%Y-%m-%dT%H:%M:%S.%fZ')
    else:
        data_dict["ngay_hieu_luc"] = None
    
    if otobaohiem.ngay_ket_thuc is not None:
        data_dict["ngay_ket_thuc"] = otobaohiem.ngay_ket_thuc.strftime('%Y-%m-%dT%H:%M:%S.%fZ')
    else:
        data_dict["ngay_ket_thuc"] = None
    data_dict["created_at"] = otobaohiem.created_at.strftime('%Y-%m-%dT%H:%M:%S.%fZ')
    return data_dict


def get_spc_tnds_oto(id_loai_hinh_bh,id_cty,trong_tai):
    cty=Company.objects.get(id_ten_cong_ty=id_cty)
    spc=GoiSanPhamChinh.objects.get(company=cty,id_loai_hinh_bao_hiem=id_loai_hinh_bh,id_trong_tai=trong_tai)
    return spc.so_phi_chinh

def get_spp_tnds_oto(id_loai_hinh_bh,id_cty,trong_tai,id_spp):
    cty=Company.objects.get(id_ten_cong_ty=id_cty)
