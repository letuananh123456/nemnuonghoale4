...
from . import utils as terml_utils
import requests
...
@app.task
def gui_thong_tin_dh_termlife(id):
    dh = DonDatHang.objects.get(id=id)
    data = terml_utils.get_data_ordertermilife(id)
    url = settings.MANAGER_URL + "termlife/create-dh-termlife/"
...
@shared_task
def filter_dh_termlife_chuagui_mana():
    ds = DonDatHang.objects.filter(loai_hinh_bao_hiem=1, is_da_gui_manager=False )
    for obj in ds:
        gui_thong_tin_dh_termlife.delay(obj.id)
