...
function HandleRightClick(obj, strEvent, strClickFor,productid,strCollection,uriString,utm_content)
{   
   if (strEvent != '' && strClickFor != '') {
        var VisitID = $('#gaData').attr('data-visit');
        try {
            dataLayer.push(
                {'event': 'eventTracking','eventCategory': 'HomeV4Click','eventAction': strClickFor,'eventLabel': VisitID,'eventValue': 0,'visitId': VisitID }
                )           
        } catch (e) {}
...
function changeRibonForNRI(strCountryName){
    ...
    if(strCountryName != 'null' && strCountryName != 'india'){
        //For old home page
        var chkMobile = false;
        ...
        //Swap tw and inv icon for home page        
        var twContent = $('.swap_dis_inv_for_nri').html();
        $(".swap_dis_tw_for_nri").html(twContent);
        $(".swap_dis_tw_for_nri div.prdIcon").removeClass('prdIcon iconItem two_wheeler_insurance').addClass('sbCateIcon two_wheeler_insurance');
        $(".swap_dis_tw_for_nri a").removeClass('links').addClass('sub-links').show();
        
        
        var objInvContent = $(".remove_for_nri");
        var invContent = objInvContent.html();
        $(".swap_dis_inv_for_nri").html(invContent);        
        $(".swap_dis_inv_for_nri div.sbCateIcon").removeClass('sbCateIcon').addClass('prdIcon');
        $(".swap_dis_inv_for_nri a").removeClass().addClass('links').show();
                
        objInvContent.html('').hide();
        if(chkMobile){
            $('.restInsurance_icon_band .group:last-child').css('display','inline-block');
            //$('.restInsurance_icon_band .group .home').parent('a').hide();
        }
        $(".container .row.products span.swap_dis_inv_for_nri > a.links").prepend('<p class="planfornri">Plans for NRI\'s</p>');
        //Swap End
...
function isNRI(){    
    $.ajax({
            ...
            success: function (countrydata) {
                if (typeof countrydata.countryName !== 'undefined') { 
                    changeRibonForNRI(countrydata.countryName);
                   if(countrydata.countryName != 'null' && countrydata.countryName != 'india'){
...
function docPrimeAd(Cip,visitid){
		...
		ip = Cip.split(",");
            ...
            $.ajax({
                url: document.location.protocol+'//' + document.location.host +'/policybazzar-home/cj-process.php?type=docprime',
                type: "POST",
                cache: false,
                async: true,
                data: postdata,
                ...
                success: function (data){
					...
					if(data.status){	
                        webImg = data.web_image_url;
                        mobImg = data.mobile_image_url;
                        windowW = $(window).width();
                        imgUrl = (windowW < 768) ? mobImg : webImg;
                       $('.docprime #docprime_img').attr('src',imgUrl);
                        $('.docprime').attr('href',data.access_url);
						$('.docprime .linkcontent').html(data.alt_text);
                        //$('.docprime').attr('onclick','getClickv3(1, "Docprime_Click","","Home-DocPrime","'+data.access_url+'")');
                        $('.docprime').attr('target','_blank');
                        $('.docprime span.loader').hide();
                }else{
                            webImg = 'https://cdn.docprime.com/static/web/images/lab_pb1.png';
                            mobImg = 'https://cdn.docprime.com/static/web/images/lab_mobile_pb1.png';
                            windowW = $(window).width();
                            imgUrl = (windowW < 768) ? mobImg : webImg;                            
                            $('.docprime #docprime_img').attr('src',imgUrl);
                            $('.docprime').attr('href','https://docprime.com');
                            $('.docprime').attr('target','_blank');
                            $('.docprime span.loader').hide();
                        }
                }}               
...
function getClickv3(strEvent, strClickFor,productid,strCollection,strRedirectUrl,utm_content) {
    if (strEvent != '' && strClickFor != '') {
        ...
        try {
            dataLayer.push(
                {'event': 'eventTracking','eventCategory': 'HomeV4Click','eventAction': strClickFor,'eventLabel': VisitID,'eventValue': 1,'visitId': VisitID }
                )           
        } catch (e) {}
		if(productid !='' && strCollection !=''){                
                pbEventTracker(productid,strCollection)                
...
function pbEventTracker(productid,strCollection){
	...
	TrackUrl = document.location.protocol+'//' + document.location.host + '/services/pbeventtracking.php';
    $.ajax({
        beforeSend: function(xhrObj) {
            xhrObj.setRequestHeader("Content-Type", "application/json");
            xhrObj.setRequestHeader("Accept", "application/json")
...
function GA360EventPage(eCategory,eAction,eLabel){
    try{         
        if(eLabel=='')
            eLabel = $('#gaData').attr('data-visit');
        dataLayer.push({
            'event': 'eventTracking', 
